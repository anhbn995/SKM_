...
def conv2d_block(input_tensor, n_filters, kernel_size=3, batchnorm=True, padding="same", activation="relu", kernel_initializer="he_normal"):
    x = input_tensor
    # first layer
    x = KL.Conv2D(filters=n_filters, kernel_size=(kernel_size, kernel_size), kernel_initializer=kernel_initializer,
                  padding=padding)(x)
    if batchnorm:
...
class BatchNorm(KL.BatchNormalization):
    """Extends the Keras KL.BatchNormalization class to allow a central place
...
def Transpose2D_block(filters, stage, kernel_size=(3, 3), upsample_rate=(2, 2),
    ...
    def layer(input_tensor):
        ...
        x = KL.Conv2DTranspose(filters, transpose_kernel_size, strides=upsample_rate,
                               padding='same', name=up_name)(input_tensor)
        if batchnorm:
            x = KL.BatchNormalization(name=bn_name+'1')(x)
        ...
        x = KL.Conv2D(filters, kernel_size, padding='same',
                      name=conv_name+'2')(x)
        if batchnorm:
            x = KL.BatchNormalization(name=bn_name+'2')(x)
...
def side_branch(x, factor):
    x = KL.Conv2D(1, (1, 1), activation=None, padding='same')(x)

    kernel_size = (2*factor, 2*factor)
    x = KL.Conv2DTranspose(1, kernel_size, strides=factor,
