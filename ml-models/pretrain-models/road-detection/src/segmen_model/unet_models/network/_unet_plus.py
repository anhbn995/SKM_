...
from segmen_model.unet_models.libs.activations_base import GELU, Snake
from segmen_model.unet_models.network._unet import UNET_downstack, UNET_upstack
from segmen_model.unet_models.libs._backbone_zoo import backbone_zoo, bach_norm_checker
from segmen_model.unet_models.libs.layer_base import decode_layer, CONV_stack, CONV_output


def unet_plus_base(input_tensor, filter_num, stack_num_down=2, stack_num_up=2,
    ...
    unet_plus_base(input_tensor, filter_num, stack_num_down=2, stack_num_up=2,
                      activation='ReLU', batch_norm=False, pool=True, unpool=True, deep_supervision=False, 
                      backbone=None, weights='imagenet', freeze_backbone=True, freeze_batch_norm=True, name='xnet')
    ...
    if backbone is None:
        ...
        for i, f in enumerate(filter_num[1:]):
            X = UNET_downstack(X, f, stack_num=stack_num_down, activation=activation,
    ...
    for nest_lev in range(1, depth_):
        ...
        for i in range(1, depth_decode):
            ...
            X_nest_skip[nest_lev].append(
                UNET_upstack(X_nest_skip[nest_lev-1][i], previous_skip, filter_num[i-1],
        ...
        if depth_decode < depth_lev+1:
            ...
            for j in range(depth_lev-depth_decode+1):
                ...
                X = UNET_upstack(X, None, filter_num[j_real-1],
                                 stack_num=stack_num_up, activation=activation, unpool=unpool,
...
def unet_plus(input_size, filter_num, n_labels, stack_num_down=2, stack_num_up=2,
    ...
    IN = Input(input_size)
    # base
    X = unet_plus_base(IN, filter_num, stack_num_down=stack_num_down, stack_num_up=stack_num_up,
    ...
    if deep_supervision:
        ...
        if (backbone is not None) and freeze_backbone:
            backbone_warn = '\n\nThe shallowest U-net++ deep supervision branch directly connects to a frozen backbone.\nTesting your configurations on `keras_unet_collection.base.unet_plus_base` is recommended.'
        ...
        OUT_list.append(CONV_output(X_list[-1], n_labels, kernel_size=1,
                        activation=output_activation, name='{}_output_final'.format(name)))

    else:
        OUT = CONV_output(X, n_labels, kernel_size=1,
                          activation=output_activation, name='{}_output'.format(name))
        OUT_list = [OUT, ]

    # model
    model = Model(inputs=[IN, ], outputs=OUT_list,
