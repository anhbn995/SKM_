...
from segmen_model.unet_models.libs.layer_base import encode_layer, decode_layer, CONV_stack, CONV_output
    ...
    if backbone is None:
        ...
        if depth_encode < depth_:
            ...
            for i in range(depth_ - depth_encode):
                ...
                X = UNET_downstack(X, filter_num_down[i_real], stack_num=stack_num_down, activation=activation,
                                   pool=pool,
...
def unet_3plus(input_size, n_labels, filter_num_down, filter_num_skip='auto', filter_num_aggregate='auto',
               stack_num_down=2, stack_num_up=1, activation='ReLU', output_activation='Sigmoid',
               batch_norm=False, pool=True, unpool=True, deep_supervision=False, multi_input=True,
               backbone=None, weights='imagenet', freeze_backbone=True, freeze_batch_norm=True, name='unet3plus'):
    '''
    ...
    * Upsampling is achieved through bilinear interpolation and can be replaced by transpose convolutional layers here.
    ...
    X_decoder = unet_3plus_base(IN, filter_num_down, filter_num_skip, filter_num_aggregate,
    ...
    if deep_supervision:
        ...
        for i in range(1, L_out):
            if not multi_input:
                pool_size = 2 ** (i)
                X = Conv2D(n_labels, 3, padding='same', name='{}_output_conv_{}'.format(
                    name, i - 1))(X_decoder[i])
                X = decode_layer(X, n_labels, pool_size, unpool,
                                 activation=None, batch_norm=False, name='{}_output_sup{}'.format(name, i - 1))
            else:
                X = Conv2D(n_labels, (1, 1), padding='same', name='{}_output_conv_{}'.format(
                    name, i - 1))(X_decoder[i])
    ...
    elif multi_input and deep_supervision:
        model = Model([IN, ], OUT_stack)
