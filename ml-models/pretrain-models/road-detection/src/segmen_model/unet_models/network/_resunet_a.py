...
from tensorflow.keras.layers import Input, add, concatenate, Conv2D
from tensorflow.keras.layers import ReLU, LeakyReLU, PReLU, ELU, Softmax
from segmen_model.unet_models.libs.activations_base import GELU, Snake
from segmen_model.unet_models.libs.layer_base import encode_layer, decode_layer, ASPP_conv, CONV_stack, CONV_output
...
def resunet_a_base(input_tensor, filter_num, dilation_num,
    ...
    * `dilation_num` should be provided as 2d iterables, with the second dimension matches the model depth.
    ...
    X = ASPP_conv(X, aspp_num_down, activation=activation,
                  batch_norm=batch_norm, name='{}_aspp_bottom'.format(name))
    ...
    X = ASPP_conv(X, aspp_num_up, activation=activation,
                  batch_norm=batch_norm, name='{}_aspp_out'.format(name))

    return X


def resunet_a(input_size, filter_num, dilation_num, n_labels,
    ...
    * `resunet_a_2d` does not support NoneType input shape.
    ...
    # base
    X = resunet_a_base(IN, filter_num, dilation_,
