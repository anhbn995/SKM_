...
from tensorflow.keras.layers import concatenate, add, Conv2D, Activation
...
def RSU(X, channel_in, channel_out, depth=5, activation='ReLU', batch_norm=True, pool=True, unpool=True, name='RSU'):
    ...
    for i in range(depth):
        ...
        X = concatenate([X, X_skip[i]], axis=-1,
    ...
    X = concatenate([X, X_skip[depth]], axis=-1,
                    name='{}_concat_out'.format(name))
...
def u2net_base(input_tensor,
    ...
    * Upsampling is achieved through bilinear interpolation in Qin et al. (2020), 
    ...
    for i, f in enumerate(filter_4f_num):
        ...
        X = concatenate([X, X_skip[tensor_count]], axis=-1,
                        name='{}_concat_4f_{}'.format(name, i))
...
def u2net(input_size, n_labels, filter_num_down, filter_num_up='auto', filter_mid_num_down='auto', filter_mid_num_up='auto',
    ...
    * Upsampling is achieved through bilinear interpolation and can be replaced by transpose convolutional layers.
    ...
    if verbose:
        ...
        print('\tNumber of RSU output channels within downsampling blocks: filter_num_down = {}'.format(
            filter_num_down))
        print('\tNumber of RSU intermediate channels within downsampling blocks: filter_mid_num_down = {}'.format(
            filter_mid_num_down))
        print('\tNumber of RSU output channels within upsampling blocks: filter_num_up = {}'.format(
            filter_num_up))
    ...
    for i in range(1, L_out):
        ...
        X = Conv2D(n_labels, 3, padding='same',
                   name='{}_output_conv_{}'.format(name, i))(X_out[i])
        ...
        if output_activation:
            if output_activation == 'Sigmoid':
                X = Activation(
                    'sigmoid', name='{}_output_sup{}_activation'.format(name, i))(X)
            else:
                activation_func = eval(output_activation)
                X = activation_func(
                    name='{}_output_sup{}_activation'.format(name, i))(X)
