...
def encode_layer(X, channel, pool_size, pool, kernel_size='auto',
    ...
    if pool == 'max':
        X = MaxPooling2D(pool_size=(pool_size, pool_size),
                         name='{}_maxpool'.format(name))(X)

    elif pool == 'ave':
        X = AveragePooling2D(pool_size=(pool_size, pool_size),
                             name='{}_avepool'.format(name))(X)
...
def attention_gate(X, g, channel,
    ...
    theta_att = Conv2D(channel, 1, use_bias=True,
                       name='{}_theta_x'.format(name))(X)
...
def ASPP_conv(X, channel, activation='ReLU', batch_norm=True, name='aspp'):
    ...
    * dilation rates are fixed to `[6, 9, 12]`.
    ...
    b4 = Conv2D(channel, 1, padding='same', use_bias=bias_flag,
    ...
    b0 = Conv2D(channel, (1, 1), padding='same',
                use_bias=bias_flag, name='{}_conv_b0'.format(name))(X)
...
def CONV_output(X, n_labels, kernel_size=1, activation='Softmax', name='conv_output'):
    ...
    X = Conv2D(n_labels, kernel_size, padding='same',
