...
from segmen_model.unet_models.losses.iou_box_seg_loss import iou_seg
from segmen_model.unet_models.losses.tversky_loss import focal_tversky
...
def segmentation_loss(y_true, y_pred, weight=None, N_CLASSES=1):
    ...
    cross_entropy_loss = cce(y_true=y_true, y_pred=y_pred)
    loss_iou = iou_seg(y_true, y_pred)
    return cross_entropy_loss + loss_iou
...
def gen_dice(y_true, y_pred, weight=None, N_CLASSES=2):
    ...
    for c in range(N_CLASSES):
        loss += dice_per_class(y_true[:, :, :, c],
                               pred_tensor[:, :, :, c])*weight
