...
class RealESRGANer():
    ...
    def process(self, img):
        # model inference
        output = self.model.predict(np.array([img[0:3],img[1:4]]))
        return np.concatenate([output[0],[output[1][2]]],axis=0)
    ...
    def enhance(self, input_path, output_path, outscale=None, alpha_upsampler='realesrgan'):
        ...
        with rasterio.open(input_path) as raster:
            meta = raster.meta
            meta.update({'count': 4, "dtype": "uint8","nodata":0})
            ...
            with rasterio.open(output_path, 'w+', **meta, compress='lzw', BIGTIFF='YES') as r:
                r.colorinterp = [ColorInterp.red, ColorInterp.green, ColorInterp.blue,ColorInterp.undefined]
                ...
                def process(coordinates):
                    ...
                    with read_lock:
                        values = raster.read([1,2,3,4],window=read_wd)
...
def run_sr(model_path,img_path,out_path,scale=4):
    ...
    upsampler.enhance(img_path, out_path, outscale=4)
