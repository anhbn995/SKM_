...
def predict(img_url, threshold = 0.5, cnn_model =None):
    ...
    for i in range(len(a)-1):
        ...
        x_batch = np.array(cut_imgs[a[i]:a[i+1]], dtype = np.float16)
        ###################################
        # fill no data with normal distributed and preprocess
        ###################################
        list_rand = list(np.random.normal(loc = 2000, scale=400, size=(100000)))
        idx_nodata = np.where(x_batch==0)
        x_batch[idx_nodata] = list_rand.pop(0)

        x_batch = x_batch/15000  # 8192
        idx_over = np.where(x_batch > 1)
        x_batch[idx_over] = 1
        ###################################
        y_batch = cnn_model.predict(x_batch)
        # if x_batch.sum() == 0: y_batch = y_batch.fill(0) # if no-data input with
...
def predict_all_2(base_path,outputFileName, cnn_model, postprocess=False):
    ...
    if postprocess:
        ...
        dilation = cv2.dilate(mask_base,kernel,iterations = 2) ## buffer shadow default = 1
...
def predict_shadow(img_data, output_im, weight = None):
