...
def BatchActivate(x):
    ...
    x = layers.LeakyReLU()(x) #  # alpha = 0.3
    return x
...
def build_model(input_shape, n_filters, DropoutRatio=0.3):
    ...
    output_layer_noActi = layers.Conv2D(3, (1, 1), padding="same", activation=None)(uconv1)
    output_layer = layers.Softmax(axis=3)(output_layer_noActi)
    ...
    # optimizer = tf.keras.optimizers.SGD(learning_rate = 1e-05, momentum = 0.9, clipnorm = True)

    from tensorflow.keras import metrics
    model.compile(optimizer=optimizer,
                loss=losses,
                metrics = ['accuracy', dice_coef, dice_coe]
    )
    return model
